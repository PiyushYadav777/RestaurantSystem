	...
	public ResponseEntity<String> generateReport(Map<String, Object> requestMap) {
		...
		try {
			...
			if (validateRequestMap(requestMap)) {
				...
				Paragraph chunk = new Paragraph("Restaurant Management", getFont("Header"));
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					addRow(table, RestaurantUtils.getMapFromJson(jsonArray.getString(i)));
				}
				document.add(table);

				Paragraph footer = new Paragraph("Total : " + requestMap.get("totalAmount") + "\n"
						+ "We're grateful for your visit. Please come back again.", getFont("Data"));
				document.add(footer);
				document.close();
